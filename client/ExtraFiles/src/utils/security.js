...
export const validateFormInput = (value, type, options = {}) => {
  switch (type) {
    case 'email':
      ...
      if (!isValidEmail(value)) return { valid: false, error: 'Please enter a valid email address' };
      return { valid: true };
    ...
    case 'text':
      ...
      if (value.length < 2) return { valid: false, error: 'Must be at least 2 characters long' };
      if (value.length > 500) return { valid: false, error: 'Must be less than 500 characters' };
      return { valid: true };
...
export const sessionUtils = {
  ...
  createSession: (userId, ttl = 3600000) => { // 1 hour default
    return {
